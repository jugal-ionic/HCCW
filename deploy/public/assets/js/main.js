...
function AgeGateScreen() {
    ...
    function validateInputs(e) {
        ...
        isSubmitting = true;
        var errorWrapper = document.getElementById("error-overlay"), errorMessage = document.getElementById("dateErrorMessage"), generalDateMessage = "You must be 18 or over to enjoy a FREE Havana Club Cuban Cocktail with us.";
...
function HomePageScreen() {
    var self = this, startDate = new Date(2016, 7, 18, 12, 0, 0, 0), now = new Date(), voucherCTA, errorMessage = document.getElementById("dateErrorMessage"), errorWrapper = document.getElementById("error-overlay"), checkDate;
    Screen.apply(this, Array.prototype.slice.call(arguments));
    ...
    function loadEnterVoucherPage(e) {
        ...
        if (now >= startDate) {
            ...
            // self.scrManager.addScreen(VoucherPageScreen);
            var errorWrapper = document.getElementById("error-overlay"), errorMessage = document.getElementById("dateErrorMessage"), voucherError = "You’re keen! It’s not quite time to enjoy a Cuban Cocktail! just yet though, you’ll have to wait until midday on the 18th, 19th and 20th of August.";
...
function MapPageScreen() {
    ...
    function searchNearAddress(address, searchFrom) {
        ...
        if (search_text === "birmingham".toLowerCase()) {
            search_text = "B1 2DS";
        } else if (search_text === "newport".toLowerCase()) {
            search_text = "NP20 1DS";
        } else if (search_text === "cambridge".toLowerCase()) {
            search_text = "CB2 1AB";
        } else if (search_text === "worcester".toLowerCase()) {
            search_text = "WR1 3LD";
        } else if (search_text === "lincoln".toLowerCase()) {
            search_text = "LN2 1JG";
        } else if (search_text === "sheffield".toLowerCase()) {
            search_text = "S9 1EP";
        } else if (search_text === "york".toLowerCase()) {
            search_text = "YO1 8AD";
        } else if (search_text === "carlisle".toLowerCase()) {
            search_text = "CA1 1RP";
        }
        // if(address==='jugal')
        // {
    ...
    this.processContainer = function() {
        this.events.publish(this.id + "ContainerReady", this);
        var backBtn = document.getElementById("backBtn"), getDirectionsBtn = document.getElementById("getDirections");
...
function PromotionOverScreen() {
    ...
    //Do post container creation processing
    this.processContainer = function() {
        ga("send", "event", "havanaClub", "click", "PromoOver");
        this.events.publish(this.id + "ContainerReady", this);
        //document.getElementById("privacy-policy-link").addEventListener("click", loadPrivacyPolicy);
        return this.container;
    };
...
function ThankYouScreen() {
    ...
    this.subscribeNewsletter = function(e) {
        ...
        ga("send", "event", "signUp", "submit", "Sign up thanks page");
    };
    //Do post container creation processing
    this.processContainer = function() {
            ...
            currentUser.signUp(null, {
                success: function(user) {
                    ...
                    if (newsletterSignup.checked) {
                        ga("send", "event", "signUp", "submit", "sign up complete");
                    }
                    signUpNewsletter();
